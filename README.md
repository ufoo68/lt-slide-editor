...
Authentication, decks are stored in PostgreSQL through Prisma.
...
Local development uses the Firebase Auth Emulator. Create `.env` from
...
continues to use the same PostgreSQL, Firebase Auth Emulator, and MinIO
volumes even if the project is launched from a different directory name.
`local:reset` and `db:reset` delete Compose volumes, so local PostgreSQL,
Firebase Auth Emulator, and MinIO data will be removed. Back up any decks
