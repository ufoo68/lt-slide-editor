...
export function Logo({ href = "/", size = "md" }: LogoProps) {
  ...
  const titleSize = size === "lg" ? "text-5xl md:text-7xl" : "text-lg";
  ...
  const content = (
    <span className="inline-flex items-center gap-3 text-foreground">
      ...
      <span className="grid leading-none">
        <span className={`${titleSize} font-black tracking-normal`}>
          LT Slide Editor
        </span>
        <span className={`${subtitleSize} mt-1 font-black uppercase tracking-[0.22em] text-mint`}>
